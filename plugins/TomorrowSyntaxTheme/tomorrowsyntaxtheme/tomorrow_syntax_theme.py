...
def load_plugin():
    syntax_manager = syntax.SyntaxThemeManager()
    syntax_manager.AddSyntaxTheme("Tomorrow", "Default Light", tomorrow)
    syntax_manager.AddSyntaxTheme("Tomorrow Night", "Default Dark", tomorrow_night)
    syntax_manager.AddSyntaxTheme("Tomorrow Night Blue", "Tomorrow Night", tomorrow_night_blue)
    syntax_manager.AddSyntaxTheme(
    ...
    syntax_manager.ApplySyntaxTheme(utils.profile_get(consts.SYNTAX_THEME_KEY,consts.DEFAULT_SYNTAX_THEME))
    shell_view = GetApp().MainFrame.GetCommonView(consts.PYTHON_INTERPRETER_VIEW_NAME)
    shell_view.text.UpdateSyntaxTheme()