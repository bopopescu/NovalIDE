...
from command import *
import queue
from strutils import path_startswith
...
class VM:
    def __init__(self):
        ...
        self._frontend_sys_path = ast.literal_eval(os.environ["NOVAL_FRONTEND_SYS_PATH"])
    ...
    def _cmd_cd(self, cmd):
        if len(cmd.args) == 1:
            ...
            try:
                ...
                raise RuntimeError("No such folder: " + path)
            except OSError as e:
                raise RuntimeError("\n".join(traceback.format_exception_only(type(e), e)))
        else:
            raise RuntimeError("cd takes one parameter")
    ...
    def _cmd_Reset(self, cmd):
        if len(cmd.args) == 0:
            ...
            raise RuntimeError("Command 'Reset' doesn't take arguments")
    ...
    def _execute_file(self, cmd, executor_class):
        ...
        if len(cmd.args) >= 1:
            ...
            raise RuntimeError("Command '%s' takes at least one argument" % cmd.name)
    ...
    def rec_format_exception_with_frame_info(etype, value, tb, chain=True):
        ...
        if chain:
            if value.__cause__ is not None:
                ...
                yield rec_format_exception_with_frame_info(None, value.__cause__, None)
                ...
                yield rec_format_exception_with_frame_info(None, value.__context__, None)
        ...
        if tb is not None:
            ...
            for entry in traceback.extract_tb(tb):
                ...
                if (
                    "noval/python/plugins/pyshell/backend" not in entry.filename
                    and r"noval\python\plugins\pyshell\backend" not in entry.filename
                    and (
...
def in_debug_mode():
    return os.environ.get("NOVAL_DEBUG", False) in [1, "1", True, "True", "true"]
...
def _report_internal_error():
    sys.stderr.write("PROBLEM WITH THONNY'S BACK-END:\n")
    traceback.print_exc()
